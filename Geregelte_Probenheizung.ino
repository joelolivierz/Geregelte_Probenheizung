...
//help variables
float shouldTemp, isTemp, stoppHeatingTemp;
...
void setup() {
  ...
  EEPROM.get(EEPROMADDRESS, shouldTemp);
  updateTemps(shouldTemp, PRINTSHOULDTEMP);
  pinMode(HEATINGCARTRIDGE, OUTPUT);
  digitalWrite(HEATINGCARTRIDGE, HIGH);
  xTaskCreatePinnedToCore(Kern0_Task, "Kern0_Task", 10000, NULL, 1, NULL, 0);
  xTaskCreatePinnedToCore(Kern1_Task, "Kern1_Task", 10000, NULL, 1, NULL, 1);
}
...
//using core 0 only to read isTemp
void Kern0_Task(void *parameter) {
  while (1) {
    isTemp = getTemp();
  }
}

//using core 1 for the rest
void Kern1_Task(void *parameter) {
  while (1) {
    ...
    if ((p.y > -1150) && (p.z < -800)) {
      if ((p.x > -1050) && (p.x < -620)) {
        if (shouldTemp > 20) {
          shouldTemp--;
          EEPROM.put(EEPROMADDRESS, shouldTemp);
          EEPROM.commit();
          updateStoppHeatingTemp();
